    ...
    <div className={`min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 ${isRTL ? 'rtl' : 'ltr'}`}>
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold text-slate-800">{t('register.title')}</h2>
          <p className="mt-2 text-gray-600">
            {isRTL ? 'انضم إلى عائلة AutoLogic اليوم' : 'Join the AutoLogic family today'}
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  {...register('firstName', { required: isRTL ? 'الاسم الأول مطلوب' : 'First name is required' })}
              ...
              <div>
                ...
                <input
                  type="text"
                  {...register('lastName', { required: isRTL ? 'اسم العائلة مطلوب' : 'Last name is required' })}
                  className={`w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent ${isRTL ? 'text-right' : 'text-left'}`}
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="terms" className={`${isRTL ? 'mr-2' : 'ml-2'} block text-sm text-gray-700`}>
                {isRTL ? (
                  <>
                    أوافق على{' '}
                    <NavLink to="/terms" className="text-yellow-600 hover:text-yellow-700">
                      الشروط والأحكام
                    </NavLink>
                    {' '}و{' '}
                    <NavLink to="/privacy" className="text-yellow-600 hover:text-yellow-700">
                      سياسة الخصوصية
                    </NavLink>
                  </>
                ) : (
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <button className="w-full inline-flex justify-center py-2 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                <span className="sr-only">{isRTL ? 'التسجيل بـ Google' : 'Sign up with Google'}</span>
              ...
              <button className="w-full inline-flex justify-center py-2 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                <span className="sr-only">{isRTL ? 'التسجيل بـ Facebook' : 'Sign up with Facebook'}</span>
